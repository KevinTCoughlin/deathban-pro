...
class Messages(private val file: File) {
    ...
    fun get(path: String): String =
        ColorUtil.colorize(config.getString(path) ?: "Missing message: $path")

    fun get(path: String, vararg replacements: Pair<String, Any>): String {
    ...
    fun prefixed(path: String, vararg replacements: Pair<String, Any>): String =
        prefix + get(path, *replacements)

    fun getBanTitle(): String = get("ban.title")

    fun getBanSubtitle(duration: Duration): String =
        get("ban.subtitle", "duration" to TimeUtil.formatDuration(duration))
    ...
    fun getWarningNearLimit(remaining: Int): String =
        prefixed("warnings.near-limit", "remaining" to remaining)

    fun getHelp(): String = get("commands.help")

    fun getCheckSelf(offenseLevel: Int, deaths: Int, max: Int): String =
        prefixed("commands.check-self",
            "offense_level" to offenseLevel,
            "deaths" to deaths,
            "max" to max
        )

    fun getCheckOther(player: String, offenseLevel: Int, deaths: Int, max: Int): String =
        prefixed("commands.check-other",
            "player" to player,
            "offense_level" to offenseLevel,
            "deaths" to deaths,
            "max" to max
    ...
    fun getCheckBanned(player: String, ban: BanRecord): String =
        prefixed("commands.check-banned",
            "player" to player,
            "return_time" to TimeUtil.formatInstant(ban.endTime)
        )

    fun getResetSuccess(player: String): String =
        prefixed("commands.reset-success", "player" to player)

    fun getPardonSuccess(player: String): String =
        prefixed("commands.pardon-success", "player" to player)

    fun getPardonNotBanned(player: String): String =
        prefixed("commands.pardon-not-banned", "player" to player)
    ...
    fun getPlayerNotFound(player: String): String =
        prefixed("errors.player-not-found", "player" to player)
